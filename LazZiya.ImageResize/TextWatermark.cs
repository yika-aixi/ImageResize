    ...
    public static class TextWatermark
    {
        ...
        public static Image AddTextWatermark(this Image img, string text, TextWatermarkOptions ops)
        {
            using (var graphics = Graphics.FromImage(img))
            {
                ...
                drawPoint.X += ops.Position.X;
                drawPoint.Y += ops.Position.Y;

                if (ops.ShadowColor.A > 0)
                {
                    Brush shadowBrush = new SolidBrush(ops.ShadowColor);
                    
                    graphics.DrawString(text, font, shadowBrush, drawPoint.X + ops.ShadowOffset.X, drawPoint.Y + ops.ShadowOffset.Y,sf);
                }
